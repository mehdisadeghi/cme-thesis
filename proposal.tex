...
of this process will be presented in the prototype and will be discussed in detail.
...
to every incoming operation. Afterwards this operation will be tracked with this id. One can image this as a ticket which
allows following every change to an operation. Apart from id we will store name of the operation and input datasets. This 
will allow us to relaunch this operation in case we need to. The store that keeps this information is distributed amoung
all participating nodes. Any further infromation such as results will be attached to this store during the process. There
are concerns running a distributed store that needs further attention and 
